...
bool check_identifiers_exist (expression const & exp, std::string const & ctx, auto const & identifiers) {
  switch (exp.type) {
    case expression::type_::function_call : 
      return check_identifiers_exist(extract_function_call(exp.data.begin(), exp.data.end()), identifiers) ;
    case expression::type_::lambda : 
      return check_identifiers_exist(extract_lambda(exp.data.begin(), exp.data.end()), identifiers) ;
    case expression::type_::identifier : 
      return check_identifiers_exist(extract_identifier(exp.data.begin(), exp.data.end()), identifiers) ;
    default : 
      return true ;
  }
}
...
bool check_identifiers_exist (identifier const & id, std::string const & ctx, auto const & identifiers) {
  return contains(identifiers.begin(), identfiers.end(), ctx + "::" + id) ;
}
...
bool check_all_identifiers (thodd const & tdd) {
  ...
  for (function const & f : tdd.functions)
    all_identifiers_exists = all_identifiers_exists && check_identifiers_exist(f) ;
